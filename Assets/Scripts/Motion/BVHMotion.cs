...
public class BVHMotion
{
    ...
    public BVHMotion(string bvhText)
    {
        this.bvhText = bvhText;
        parse(false, 0f);
    }

    public BVHMotion(string bvhText, float time)
    {
        this.bvhText = bvhText;
        parse(true, time);
    }
    ...
    public class BVHBone
    {
        ...
        public BVHBone(BVHMotion parser, bool rootBone)
        {
            bp = parser;
        ...
        public Vector3 GetLocalPosition(float frameIndex)
        {
        ...
        public Quaternion GetLocalQuaternion(float frameIndex)
        {
            int low = Mathf.FloorToInt(frameIndex);
            int high = Mathf.CeilToInt(frameIndex);
            float alpha = frameIndex - low;
    ...
    {
        return new BVHMotion(this.bvhText);
    }
    ...
    public static Vector3 FindTotalOffset(BVHBone bone)
    {
        ...
        if (bone.parent != null)
        {
            result += BVHMotion.FindTotalOffset(bone.parent);
        }
    ...
    private void handle()
    {
        ...
        foreach (BVHMotion.BVHBone bone in this.allBones)
        {
